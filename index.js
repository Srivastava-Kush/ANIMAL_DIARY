...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Register flipbook for global usage
  window.initFlipbook = initFlipbook;
  window.resetFilter = resetFilter;

  const enterGlobeButton = document.getElementById("enterGlobeButton");
  if (enterGlobeButton) {
    enterGlobeButton.addEventListener("click", enterGlobe);
  }
});
...
function addAnimalSprite(animal) {
  ...
  if (
    ["critically_endangered", "endangered", "vulnerable"].includes(
      animal.iucn_status
    )
  ) {
...
function loadAnimalImage(imgUrl, sprite) {
  ...
  if (imgUrl.includes("commons.wikimedia.org")) {
    // Make a direct request to the media API to get the actual image URL
    modifiedUrl = `https://commons.wikimedia.org/w/api.php?action=query&titles=File:${filename}&prop=imageinfo&iiprop=url&format=json&origin=*`;
    ...
    fetch(modifiedUrl)
      ...
      .then((data) => {
        ...
        if (firstPage.imageinfo && firstPage.imageinfo[0]) {
          const actualImageUrl = firstPage.imageinfo[0].url;
          loadImageWithUrl(actualImageUrl, sprite);
        }
      })
      .catch((error) => {
        console.warn(
          `Could not load image info for ${sprite.userData.animal.name}:`,
          error
        );
      });
    return;
  ...
  loadImageWithUrl(modifiedUrl, sprite);
}

function loadImageWithUrl(url, sprite) {
  const textureLoader = new THREE.TextureLoader();
  textureLoader.crossOrigin = "anonymous";
  textureLoader.load(
    url,
    ...
    (error) => {
      console.warn(
        `Could not load image for ${sprite.userData.animal.name} from URL ${url}:`,
        error
      );
      // Keep the fallback circular marker with initial - no need to do anything as it's already there
    }
  );
}
  ...
  modalBody.innerHTML = `
        <div class="animal-detail">
            ...
            <p><strong>Fun Fact:</strong> ${
              animal.fun_fact || "No fun fact available"
            }</p>
            <button onclick="showAllOccurrences('${
              animal.name
            }')" class="occurrence-btn">
                Show All Occurrences
            </button>
            <button onclick="reportDiscrepancy('${
              animal.name
            }')" class="report-btn">
                Report Discrepancy
            </button>
